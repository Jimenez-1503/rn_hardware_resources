...
export default function WiFiScreen() {
  ...
  const typeLabel = networkState.type != null ? NetworkTypeLabel[networkState.type] : '-';
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <Section title="Estado da conexao">
        <InfoRow
          label="Conectado"
          value={networkState.isConnected == null ? '-' : networkState.isConnected ? 'Sim' : 'Nao'}
      ...
      <Section title="Informacoes do dispositivo">
        ...
        <InfoRow
          label="Modo aviao"
          value={airplaneMode == null ? '-' : airplaneMode ? 'Ativado' : 'Desativado'}
        />
      </Section>
      ...
      <TouchableOpacity
        ...
        activeOpacity={0.8}
      >
        <Text style={styles.buttonText}>{loading ? 'Atualizando...' : 'Atualizar informacoes'}</Text>
      </TouchableOpacity>

      <Text style={styles.hint}>
        O estado da conexao e atualizado automaticamente ao detectar mudancas de rede.
      </Text>
    </ScrollView>
  );
}
...
function InfoRow({ label, value, highlight }) {
  return (
    <View style={styles.row}>
      ...
      <Text style={[styles.value, highlight === true && styles.valueGreen, highlight === false && styles.valueRed]}>
        {String(value)}
      </Text>
    </View>
  );
